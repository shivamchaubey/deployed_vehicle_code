...
class PathFollowingLPV_MPC:
    ...
    def MPC_update(self, A_vec, B_vec, x0, ref):
        ...
        # print "self.A_row_indices, self.A_col_indices", self.A_row_indices, self.A_col_indices
        # print "Ax_value", Ax_value
...
def main():
    ...
    # rospy.init_node('optimizer_predictor', anonymous=True)
    ...
    vx_est = vx_est[start:end]
    vy_est = vy_est[start:end]
    omega_est = omega_est[start:end]

    GlobalState = np.array([vx_est[0], vy_est[0], omega_est[0], x_est[0], y_est[0], yaw_est[0]])
    ...
    for i in range(len(x_est)):
        ...
        GlobalState = np.array([vx_est[i], vy_est[i], omega_est[i], x_est[i], y_est[i], yaw_est[i]]).T
        LocalState = GlobalState
        ...
        # Controller.uPred[0,:] = [steer[i], duty[i]]

        # print LocalState[0:6]
        ...
        print 'Controller feasible', Controller.feasible

        control_pred.extend([LPV_States_Prediction])
        control_opti.extend([Controller.xPred])
        control_cont.extend([Controller.uPred])
    ...
    control_opti = np.array(control_opti)
    control_cont = np.array(control_cont)

    print 'Controller.uPred', Controller.uPred.shape
    print  'Controller.xPred', Controller.xPred.shape
    print 'control_opti', control_opti.shape
    print 'control_pred.shape',control_pred.shape
    print 'LPV_States_Prediction.shape', LPV_States_Prediction.shape
    print 'counter',counter

    # control_pred = control_pred.reshape(counter, LPV_States_Prediction.shape[0],LPV_States_Prediction.shape[1] )
    # control_opti = control_opti.reshape(counter, Controller.xPred.shape[0], Controller.xPred.shape[1])
    # control_cont = control_cont.reshape(counter, Controller.uPred.shape[0], Controller.uPred.shape[1] )


    control_dict = {'pred': control_pred, 'opti': control_opti, 'cont': control_cont}
    path = '/home/auto/Desktop/autonomus_vehicle_project/project/development/proto/plotter/log'
    dir_name = '/estimator'
    np.save(path+dir_name + '/mpc.npy', control_dict)
    print "Saved"
