...
class PathFollowingLPV_MPC:
    ...
    def __init__(self, N, vx_ref, dt, map):
        ...
        self.slew_rate_on = rospy.get_param("/planner/integral_action")
        self.soft_constraints_on = rospy.get_param("/planner/soft_constraints")
        ...
        if self.planning_mode == 1:            
            ...
            self.Qw  = 0.6 * np.array([0.8*vx_scale, 0.001, 0.001, 0.0908*etheta_scale, 0.0, 0.08*ey_scale]) # penality on states 
            self.Rw  = 0.1 * np.array([0.05*str_scale, 0.05*duty_scale])     # Penality on input (dutycycle, steer)


            if self.slew_rate_on:
                self.Qw  = 0.7 * np.array([0.3*vx_scale, 0.0001, 0.0001, 0.02*etheta_scale, 0.0, 0.08*ey_scale]) # penality on states 
    ...
    # def MPC_full(self, A_vec, B_vec, u_ref, x0, x_ref):
    ...
    def LPVPrediction_setup(self):
        ...
        for i in range(0, self.N):
            ...
            Ai = np.array([ [1.0    ,  1.0 ,   1.  ,  0., 0., 0.],  # [vx]
                            [0.0    ,  1.0 ,   0.0   ,  0., 0., 0.],  # [vy]
                            [0.0    ,   1.0  ,   1.0   ,  0., 0., 0.],  # [wz]
                            [1.0    ,  1.0 ,   1.0 ,  0., 0., 0.],  # [epsi]
                            [1.0    ,  1.0 ,   0.  ,  0., 0., 0.],  # [s]
                            [1.0    ,  1.0 ,   0.  ,  0., 0., 0.]]) # [ey]
