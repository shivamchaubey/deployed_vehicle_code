...
marvel_frame = '/marvel_frame'
imu_frame = '/imu_frame'
fcam_frame = '/fcam_frame'
lidar_frame = '/lidar_frame'
kinect_frame = '/kinect_frame'

loc1 = [0,0] #world frame
loc2 = [1.25,-1.34]   #world frame 2 near the table
...
class motor_encoder():
    ...
    def RPM_callback(self, data):
        ...
        self.s        += self.vx*(self.curr_time - self.prev_time)  
...
class marvelmind():
    ...
    def __init__(self,t0,N):
        ...
        # self.t      =  tf.Transformer(True, rospy.Duration(5.0))
        # self.m = geometry_msgs.msg.TransformStamped()
        # self.m.header.frame_id = fixed_frame
        # self.m.child_frame_id = marvel_frame
        # # self.m.transform.translation.x = 2.71828183
        # # self.m.transform.translation.y = 2.71828183
        # # self.m.transform.translation.z = 2.71828183
        # self.m.transform.rotation = Quaternion(*tf.transformations.quaternion_from_euler(0, 0, pi)) #RPY --> Quaternion
        # self.t.setTransform(m)
        # self.t.getFrameStrings()
...
class fiseye_cam():
    ...
    def fused_cam_pose_callback(self, data):
        ...
        Vx_MA = (Gvx_MA*cos(self.yaw_MA)+Gvy_MA*cos(self.yaw_MA))
        Vy_MA = (-Gvx_MA*sin(self.yaw_MA)+Gvy_MA*cos(self.yaw_MA))

        Vx = (Gvx*cos(self.yaw)+Gvy*cos(self.yaw))
