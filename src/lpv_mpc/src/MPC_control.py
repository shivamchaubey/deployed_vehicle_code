...
#########################################################
    ...
    #maximum values
    vx_max               = rospy.get_param("max_vel")
    vx_min               = rospy.get_param("min_vel")
    Dc_max              = rospy.get_param("dutycycle_max")
    Dc_min              = rospy.get_param("dutycycle_min")
    ...
    vx_scale            = 1/((vx_max-vx_min)**2)
    Dc_scale           = 1/((Dc_max-Dc_min)**2)
    ...
    dDc_scale          = 1/((dDc_max-dDc_min)**2)



    Q  = 0.9 * np.diag([0.6*vx_scale, 0.0, 0.001, 0.1*etheta_scale, 0.0, 0.6*ey_scale])
    R  = 0.05 * np.diag([0.05*str_scale,0.1*Dc_scale])     # delta, a
    dR = 0.1 * np.array([0.01*dstr_scale,0.01*dDc_scale])  # Input rate cost u
    ...
    while (not rospy.is_shutdown()):
        ...
        if first_it < 10:
            ...
            if Controller.feasible == 0:
                print "Controller feasible"

                controller_Flag_msg.data = True

                controller_Flag.publish(controller_Flag_msg)
            else:
                print "Controller not feasible"
                controller_Flag_msg.data = False
                controller_Flag.publish(controller_Flag_msg)


        print('control actions',Controller.uPred[0,0],Controller.uPred[0,1])
        print('\n')
        ...
        if Controller.uPred[0,1]< 0.3:
            print "crossed safe limit of dutycylce"
            accel_commands.publish(Controller.uPred[0,1])

        else:
            accel_commands.publish(0.0)

        steering_commands.publish(Controller.uPred[0,0])

        # print "\n Controller.xPred shape", (Controller.xPred).shape, "\n Controller.xPred", Controller.xPred
